...
-- ============================================================
-- CASH REGISTER X AUTOMATIC - NEW DATABASE SETUP
-- Target project:
-- https://typnceqlgrcpkokulnkp.supabase.co
--
-- Run this ONE file in Supabase SQL Editor.
-- It creates/migrates the CRX tables and applies the V9.1
-- columns/policies/auth helpers without deleting business data.
-- ============================================================
...
-- ------------------------------------------------------------
-- Ensure the current Admin Auth account has an application
-- profile. If the Auth user already exists, this creates/updates
-- only its public.profiles row.
-- ------------------------------------------------------------
do $$
declare
  v_id uuid;
...
begin
  ...
  from auth.users
  where lower(email) = lower('cjgallaza09@gmail.com')
  limit 1;

  if v_id is not null then
    insert into public.profiles (id, email, full_name, role, username)
    values (v_id, v_email, 'Admin', 'admin', 'cjgallaza09')
...
-- Verify the application profile.
